...
export const getOrderStatusBreakdown = async (
  ...
  try {
    ...
    const breakdown: OrderStatusBreakdown[] = Object.entries(statusCounts).map(([status, count]) => ({
      status,
      count,
...
export const getOrderPerformanceMetrics = async (
  ...
  try {
    ...
    const totalOrders = data?.length || 0;
    const completedOrders = data?.filter(order => order.order_status === 'Picked up').length || 0;
    const cancelledOrders = data?.filter(order => order.order_status === 'Cancelled').length || 0;
    ...
    const processedOrders = data?.filter(order => order.order_status === 'Picked up' && order.updated_at) || [];
    ...
    if (processedOrders.length > 0) {
      ...
      avgProcessingTime = Math.round((totalProcessingDays / processedOrders.length) * 10) / 10;
    }
