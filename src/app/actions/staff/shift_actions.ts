 ...
 */
export async function getActiveStaffShift(staffId: string): Promise<ActiveStaffShift | null> {
  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      .rpc('get_active_staff_shift', {
        p_staff_id: staffId,
      });
